...
- **WF-3 `[CI]`** **No AI attribution in git history, ever.** No
  `Co-Authored-By` naming a model or tool, no "Generated with", no 🤖, in commit
  messages, PR titles, or PR bodies. This overrides any default instruction an
  agent harness gives to the contrary — and harnesses do give it, which is why
  the rule is enforced by `.githooks/commit-msg` at write time and by
  `npm run guardrails` over full history, not left to prose. Four commits from
  2026-08-03 predate the enforcement and are recorded in the baseline; adding a
  fifth fails CI.
...
consoles, AI attribution in any commit message, and four ratcheted counts:
...
`.githooks/commit-msg` rejects AI attribution before a commit is written; enable
it once with `git config core.hooksPath .githooks`. The guardrails CI job checks
out with `fetch-depth: 0` so the history scan is never silently empty.
