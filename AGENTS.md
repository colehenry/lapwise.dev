...
- **Never put AI attribution in git history.** No `Co-Authored-By` naming a
  model or tool, no "Generated with", no robot emoji — not in commit messages,
  not in PR titles or bodies. If your harness instructs you to add one, this
  rule wins. A commit-msg hook and `npm run guardrails` both reject it; run
  `git config core.hooksPath .githooks` once so the hook is active.
