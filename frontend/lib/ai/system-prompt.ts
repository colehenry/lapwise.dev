...
3. Use one relevant typed context tool before raw SQL. Use get_race_strategy_insights for undercut attempts, position margins around pit stops, or double-stacks. For any full-season summary, trend, data-point, or visual question, use get_season_context; it already returns standings, margins, and charts.
...
8. Never infer race shape from the podium, grid, or final margin alone. Pit duration means time from pit entry to pit exit, never only stationary service or pit-crew time. A per-driver pace intercept describes the car-driver entry, not the car or driver in isolation.
