...
describe("getSeasonContext", () => {
  ...
  it("returns one compact bundle with links, signed margins, and charts", async () => {
    ...
    const streamed = await getSeasonContext.execute!({ season: 2026 }, {
      toolCallId: "test",
      messages: [],
    } as never);

    /* `execute` is typed as the bundle or an async iterable of it; this tool
       resolves to the bundle, so the test says so before reading into it. */
    if (Symbol.asyncIterator in streamed) throw new Error("expected a bundle");
