...
class LapsService:
    ...
    async def get_sprint_lap_times(
        ...
        race_control_events = await SessionDataService.get_race_control_events(
            db, session.id
        )

        total_laps = max(row.lap_number for row in lap_rows) if lap_rows else None

        durations = await pit_durations(db, session.id)
        ...
        for row in lap_rows:
            ...
            drivers_dict[driver_id]["laps"].append(
                LapData(
                    ...
                    pit_duration_seconds=durations.get((row.driver_id, row.lap_number)),
        ...
        for row in lap_rows:
            ...
            drivers_dict[driver_id]["laps"].append(
                LapData(
                    ...
                    is_personal_best=row.is_personal_best,
                    deleted=row.deleted,
                    lap_start_time_seconds=sanitize_float(row.lap_start_time_seconds),
                )
            )
